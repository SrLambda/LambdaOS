...
Usar glifos Unicode o iconos mono-lineales simples. Sin dependencia de sets de iconos externos para el prototipo; en Figma se pueden usar iconos genéricos del sistema.

| Concepto | Símbolo sugerido |
|----------|-----------------|
| Cursor / seleccionado | `▶` o `●` |
| Toggle encendido | `●` (relleno, color éxito) |
| Toggle apagado | `○` (vacío, color secundario) |
| Expandir / abrir | `▶` (gira a `▼`) |
| Ayuda | `?` |
| Cerrar / volver | `✕` o `←` |
| Advertencia | `⚠` |
| Candado / requiere root | `🔒` |
